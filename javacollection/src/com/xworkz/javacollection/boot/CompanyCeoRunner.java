...
public class CompanyCeoRunner {
	...
	public static void main(String[] args) {
		...
		ref.stream().map(dto -> dto.getDaughterDTO()).collect(Collectors.toList())
				.forEach(vet -> System.out.println(vet));
		System.out.println(System.lineSeparator());

		ref.stream().map(dto -> dto.getDaughterDTO().getName()).collect(Collectors.toSet())
				.forEach(ele -> System.err.println(ele));
		System.out.println(System.lineSeparator());
		ref.stream().filter(values -> values.getAge() > 30).collect(Collectors.toList())
				.forEach(dto -> System.out.println(dto));
		System.out.println(System.lineSeparator());

		ref.stream().forEach(age -> System.err
				.println("Father age:" + age.getAge() + "  Daughter age:" + age.getDaughterDTO().getAge()));
	}
